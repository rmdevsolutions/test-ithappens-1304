...
if ($_SERVER['REQUEST_METHOD'] == 'GET')
{
    ...
    if ($requestHTTP == 'insertItensPedido')
    {
        ...
        $sqlProdutoQuantidade = $conecta->query($sql) or die(json_encode(array(
            'mensagem' => 'erro',
            'erro' => $conecta->error
        )));
        ...
        $retorno = $conecta->query($busca) or die(json_encode(array(
            'mensagem' => 'erro',
            'erro' => $conecta->error
        )));
        if ($retorno->num_rows == 0)
        {
            echo json_encode(array(
                'mensagem' => 'erro',
                'erro' => 'Produto não encontra-se no Estoque',
                'quantidadeEstoque' => 0
            ));
            exit();
        }
        ...
        ON DUPLICATE KEY UPDATE Quantidade = Quantidade + $quantidadeProduto;";
            ...
            else
            {
                echo json_encode(array(
                    'mensagem' => 'erro',
                    'erro' => 'Estoque com quantidade inferior ao solicitado',
                    'quantidadeEstoque' => ($quantidadeEmEstoque - $quantidadeEmListaPedido + $quantidadeProduto)
                ));
            }
        }
    }
        ...
        if ($retornoCON->num_rows > 0)
        {
            while ($r = $retornoCON->fetch_assoc())
            {
                ...
                if ($conecta->query($query_))
                {
                    ...
                    $conecta->query($query) or die(json_encode(array(
                        'mensagem' => 'erro',
                        'erro' => $conecta->error
                    )));
                }
